...
from chatbot import analyze_sentiment, answer   # Importando a função 'answer' e 'analyze_sentiment' do arquivo chatbot.py
...
def enviar_pergunta():
    ...
    if pergunta.strip(): 
        ...
        sentimento = analyze_sentiment(pergunta)
        sentimento_texto = (
            f"Sentimento: {sentimento['sentimento']}\n"
            f"Positividade: {sentimento['positividade']}\n"
            f"Negatividade: {sentimento['negatividade']}\n"
            f"Neutralidade: {sentimento['neutralidade']}"
        )
        adicionar_mensagem(sentimento_texto)  # Mostra o sentimento na interface
        
        resposta = answer(pergunta)  # Chama a função 'answer' para obter a resposta
...
chat_area = ctk.CTkTextbox(frame, font=("Helvetica", 14), fg_color=cor_base,
                           text_color="white", corner_radius=10, state="disabled", height=400)
...
botao = ctk.CTkButton(entry_frame, image=foto_papel_aviao, command=enviar_pergunta,
                      fg_color=cor_terciaria, hover_color="#008C8C", width=50, text="")
